...
void StabPolarDlg::readData()
{
	if(m_pctrlUnit1->isChecked())
	{
		s_StabWPolar.density()   = m_pctrlDensity->value();
		s_StabWPolar.viscosity() = m_pctrlViscosity->value();
	}
	else
	{
		s_StabWPolar.density()   = m_pctrlDensity->value()   / 0.00194122;
		s_StabWPolar.viscosity() = m_pctrlViscosity->value() / 10.7182881;
	}
...
void StabPolarDlg::onAeroData()
{
	...
	{
		s_StabWPolar.density() = dlg.AirDensity();
		s_StabWPolar.viscosity() = dlg.KinematicViscosity();
